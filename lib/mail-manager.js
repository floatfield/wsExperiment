...
MailManager = (function() {
  ...
  MailManager.prototype.sendWelcomeLetter = function(email, password) {
    var config, locals;
    config = this.getTransporterConfig(email, 'Добро пожаловать');
    locals = {
      password: password,
      login: email
    };
    return this.mailer.sendEmail('welcome-letter', config, locals);
  };
